...
if __name__ == "__main__":
    print("="*60)
    print("PENG-ROBINSON EOS - FIXED VERSION")
    print("="*60)

    T = 420.0  # K
    P_bar = 2.0
    P = P_bar * BAR_TO_PA  # Pa
    z = np.array([0.5, 0.5])
    
    names = ["n-C10H22", "n-C8H18"]
    components = [
        {
            "name": "n-C10H22",
            "Tc": 617.70,
            ...
            "omega": 0.488,
            "MW": 142.286,
            "Cp": (2.0387e2, 1.0055e-1, -3.0176e-5, 3.4531e-9, 0.0),
        },
        {
            "name": "n-C8H18",
            "Tc": 568.70,
            ...
            "omega": 0.399,
            "MW": 114.232,
            "Cp": (1.7735e2, 8.3230e-2, -2.1990e-5, 2.2350e-9, 0.0),
        }
    ]

    kij = np.zeros((2, 2))
    Tref = 298.15  # K
    Pref = 1.0 * BAR_TO_PA  # Pa

    result = industrial_TP_equilibrium_PR(T, P, z, components, kij, Tref, Pref, do_ideal=True, verbose=True)
    ...
    if result["type"] == "single-phase":
        ...
        print(f"Liquid fraction       = {1-flash['beta']:.6f}")
        print(f"\nLiquid composition (x): {flash['x']}")
